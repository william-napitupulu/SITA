...
class AuthServiceProvider extends ServiceProvider
{
    ...
    /**
     * Register the access gate service and define the isAdmin gate.
     *
     * @return void
     */
    protected function registerAccessGate()
    {
        $this->app->singleton(GateContract::class, function ($app) {
            $gate = new Gate($app, fn () => call_user_func($app['auth']->userResolver()));

            // Define the 'isAdmin' gate to restrict access to admin-only sections
            $gate->define('isAdmin', function ($user) {
                return $user->role === 'Admin'; // Change 'Admin' based on your role naming
            });
